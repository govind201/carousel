...
const AudioFeatures = ({ token, userId }) => {
  ...
  //
  const { shortTerm, mediumTerm, longTerm, loading } = useFetchAll({
    urls: urls,
    token: token,
  });

  React.useEffect(() => {
    let shortTermArr = shortTerm
      .map((item) => item.id)
  ...
  React.useEffect(() => {
    ...
    if (audioFeaturesLongTerm.length === 50 || loading) {
      console.log(
        "still loading tracks or tracks not loaded",
        loading,
        longTerm
      );
  ...
  React.useEffect(() => {
    ...
    console.log(
      "filterredIdarr in useEffect for fetching top tracks for the current mood",
      filteredIDArr
    );
    let tempFilterArr = filteredIDArr;
    tempFilterArr.splice(12);
    let arrLink = tempFilterArr
      .map((item) => item.id)
  ...
  React.useEffect(() => {
    const select_tracks = (track) => {
      ...
      if (energy === "low" && acousticness === "high" && tempo < 100) {
        return "Chill";
        ...
        return "Feelz";
      } else if (danceability !== "low" && tempo > 100 && energy !== "low") {
        return "Party";
      } else {
        return "none";
      }
    ...
    console.log(
      "Data loaded and useEffect for populating filtered id arr called"
    );
    if (viewNum === "All Time") {
      setFilteredIDArr(filterSongs(audioFeaturesLongTerm));
    } else if (viewNum === "6 Months") {
      setFilteredIDArr(audioFeaturesMediumTerm);
    } else {
      setFilteredIDArr(audioFeaturesShortTerm);
    }
  }, [
    Mood,
    audioFeaturesLongTerm,
  ...
  React.useEffect(() => {
    if (
      audioFeaturesLongTerm.length === 50 &&
      audioFeaturesMediumTerm.length === 50 &&
      audioFeaturesShortTerm.length === 50
    )
    ...
    audioFeaturesLongTerm.length,
    audioFeaturesMediumTerm.length,
    audioFeaturesShortTerm.length,
  ]);
        ...
        <div>
          <div style={{ textAlign: "center" }}>
            <ButtonDropdown
              ...
              <DropdownMenu>
                <DropdownItem onClick={clickedAcoustic}>
                  Acoustic <Emoji text=":guitar:" />
                </DropdownItem>
                <DropdownItem onClick={clickedChill}>
                  Chill <Emoji text=":sun_behind_cloud:" />
                </DropdownItem>
                <DropdownItem onClick={clickedHappy}>
                  Happy <Emoji text=":upside_down:" />
                </DropdownItem>
                <DropdownItem onClick={clickedSad}>
                  Feelz <Emoji text=":disappointed_face:" />
                </DropdownItem>
                <DropdownItem onClick={clickedParty}>
                  Party <Emoji text=":dancer:" />
                </DropdownItem>
                <DropdownItem onClick={clickedAll}>
                  All Songs <Emoji text=":musical_notes:" />
                </DropdownItem>
              </DropdownMenu>
            </ButtonDropdown>
            ...
            >
              ...
              <DropdownMenu>
                <DropdownItem onClick={clickedLT}>
                  All Time <Emoji text=":hourglass_not_done:" />
                </DropdownItem>
                <DropdownItem onClick={clickedMT}>
                  6 Months <Emoji text=":calendar:" />
                </DropdownItem>
                <DropdownItem onClick={clickedST}>
                  30 Days <Emoji text=":clock1:" />
                </DropdownItem>
              </DropdownMenu>
            </ButtonDropdown>
